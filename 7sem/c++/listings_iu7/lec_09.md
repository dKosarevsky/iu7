...
```
Пример 08.11. Правило вызова функций.
```cpp
template <typename Type>
...
```
Пример 08.12. Определение типа возвращаемого значения для шаблона функции.
```cpp
...
```
Пример 08.2. Шаблон класса, шаблоны методов.
```cpp
...
```
Пример 08.14. Шаблонный метод класса.
```cpp
class A
{
public:
...
```
Пример 08.15. Шаблонный метод шаблонного класса.
```cpp
template<typename T>
class A
{
...
```
Пример 08.16. Использование decltype(auto) на примере шаблонного класса Complex.
```cpp
template<typename T>
class Complex
{
...
public:
    ...
    template<typename U>
    decltype(auto) operator+(const Complex<U>& d) const;

    const T& getReal() const { return real; }
    const T& getImag() const { return imag; }

    template<typename Type>
    friend ostream& operator<<(ostream& os, const Complex<Type>& com);
};

template<typename T>
template<typename U>
decltype(auto) Complex<T>::operator+(const Complex<U>& d) const
{
    return Complex<decltype(d.getReal() + this->real)>(d.getReal() + this->real, d.getImag() + this->imag);
}

template<typename Type>
ostream& operator<<(ostream& os, const Complex<Type>& com)
{
    return os << "( " << com.real << ", " << com.imag << ")" << endl;
}
...
```
Пример 08.3. Полная специализация шаблона класса и метода шаблона класса.
```cpp
template <typename Type>
class A
{
...
```
Пример 08.4. Частичная специализация шаблона класса, параметры шаблона класса по умолчанию.
```cpp
...
```
Пример 08.5. Шаблон функции с переменным числом параметров.
```cpp
template <typename Type>
Type sum(Type value)
{
...
```
Пример 08.6. Шаблон с переменным числом параметров значений.
```cpp
template<size_t...>
struct Sum {};
...
```
Пример 08.7. Шаблон класса с переменным числом параметров. Рекурсивная реализация кортежа.
```cpp
...
```
Пример 08.13. Приведение типов в С++. 
```cpp
class A
{
	int a = 0;
...
```
Пример 08.8. Реализация хранителя unique_ptr. 
```cpp
template <typename Type>
class UniquePtr
{
...
```
Пример 08.9. Реализация shared_ptr и weak_ptr. 
```cpp
# include "UniquePtr.h"
...
```
Пример 08.10. Создание итератора (без проверок и обработки исключительных ситуация). 
```cpp
# include <iostream>
# include <memory>
# include <iterator>
